...
const AssignedProjectCard = ({ name, deadline, assignedTo, id} : {
  ...
  return (
    <div onMouseOver={()=>handleHover(id)} onClick={()=>handleNavigate(id)} style={{boxShadow: '3px 3px 5px rgba(0, 0, 0, .05)'}} className="auto cursor-pointer p-4 bg-[#fff] rounded-[6px] border m-2">
        <h1 className="text-[#444] text-[15.84px] font-bold">{name}</h1>
        <h3 className="text-[#999] text-[15.84px]">Due by: {deadline}</h3>
       <div className="mt-5 border-t border-t-[#eee] flex pt-5 overflow-x-auto">
        {
